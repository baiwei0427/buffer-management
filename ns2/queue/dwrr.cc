...
DWRR::DWRR(): timer_(this)
{
	...
	buffer_scheme_ = STATIC_PORT_THRESH;
	shared_buffer_id_ = 0;
	alpha_ = 1;
	debug_ = 0;
	...
	bind("buffer_scheme_", &buffer_scheme_);
	bind("shared_buffer_id_", &shared_buffer_id_);
	bind("alpha_", &alpha_);
...
/* Determine whether we need to mark ECN where q is the index of the queue. Return 1 if it requires marking */
int DWRR::MarkingECN(int q)
{
	...
	{
		if (queues[q].byteLength() > queues[q].ecn_thresh * mean_pktsize_)
			return 1;
		else
			return 0;
	}
...
/* Determine whether to drop this packet. q is the index of the queue. pkt_len is the length of the packet. Return 1 if the packet gets dropped. */
int DWRR::DropPacket(int q, int pkt_len)
{
	if (q < 0 || q >= queue_num_)
	{
		fprintf (stderr,"illegal queue index\n");
	...
	{
		if (pkt_len + TotalByteLength() > qlim_ * mean_pktsize_)
			return 1;
		else
			return 0;
	}
	else if (buffer_scheme_ == STATIC_THRESH)
	{
		if (pkt_len + queues[q].byteLength() > queues[q].queue_thresh * mean_pktsize_ || pkt_len + shared_buf_bytes_[shared_buffer_id_] > shared_buf_lim_[shared_buffer_id_])
			return 1;
		else
			return 0;
	}
	...
	else if (buffer_scheme_ == MQ_DYNAMIC_THRESH)
	{
		double port_buffer_lim = alpha_ * (shared_buf_lim_[shared_buffer_id_] - shared_buf_bytes_[shared_buffer_id_]);
		double queue_buffer_lim = queues[q].alpha * (port_buffer_lim - TotalByteLength());
		if (pkt_len + queues[q].byteLength() > queue_buffer_lim)
			return 1;
		else
			return 0;
	}
	...
	{
		fprintf (stderr,"Unknown buffer management scheme\n");
		return 0;
	}
}
...
int DWRR::command(int argc, const char*const* argv)
{
	...
	else if (argc==4)
	{
		...
		else if (strcmp(argv[1], "set-ecn-thresh") == 0)
		{
			int queue_id = atoi(argv[2]);
			if (queue_id < queue_num_ && queue_id >= 0)
			{
				double ecn_thresh = atof(argv[3]);
				if (ecn_thresh >= 0)
				{
					queues[queue_id].ecn_thresh = ecn_thresh;
					return (TCL_OK);
				}
				else
				{
					fprintf(stderr, "illegal ECN threshold value %s for queue %s\n", argv[3],argv[2]);
					exit(1);
				}
			}
			/* Exceed the maximum queue number or smaller than 0*/
		...
		else if (strcmp(argv[1], "set-queue-thresh") == 0)
		{
			int queue_id = atoi(argv[2]);
			if (queue_id < queue_num_ && queue_id >= 0)
			{
				double queue_thresh = atof(argv[3]);
				if (queue_thresh >= 0)
				{
					queues[queue_id].queue_thresh = queue_thresh;
					return (TCL_OK);
				}
				else
				{
					fprintf(stderr, "illegal queue length threshold value %s for queue %s\n", argv[3],argv[2]);
					exit(1);
				}
			}
			/* Exceed the maximum queue number or smaller than 0*/
		...
		else if (strcmp(argv[1], "set-alpha") == 0)
		{
			int queue_id = atoi(argv[2]);
			if (queue_id < queue_num_ && queue_id >= 0)
			{
				...
				if (alpha > 0)
				{
					queues[queue_id].alpha = alpha;
					return (TCL_OK);
				}
				else
				{
					fprintf(stderr, "illegal alpha value %s for queue %s\n", argv[3],argv[2]);
					exit(1);
				}
			}
			/* Exceed the maximum queue number or smaller than 0*/
